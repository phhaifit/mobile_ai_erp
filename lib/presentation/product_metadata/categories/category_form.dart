...
class _ProductMetadataCategoryFormScreenState extends State<ProductMetadataCategoryFormScreen> {
  ...
  Future<void> _initialize() async {
    final results = await Future.wait([
      _store.loadCategoryParentOptions(),
      if (widget.args?.categoryId != null)
        _store.getCategoryById(widget.args!.categoryId!).then<Category?>((c) => c).catchError((_) => null),
    ]);
    if (widget.args?.categoryId != null) {
      _editingCategory = results.last as Category?;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Observer(
        builder: (_) {
          final parentOptions = _store.categoryParentOptions.toList()
            ..sort((a, b) => _label(a).toLowerCase().compareTo(_label(b).toLowerCase()));
          return CategoryFormBody(
            formKey: _formKey,
            ...
            onSave: _save,
            validateSlug: _validateSlugFormat,
          );
        },
      ),
    );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(MetadataErrorFormatter.formatActionError(
            error: error,
            actionLabel: _editingCategory == null ? 'create category' : 'save category',
          )),
        ));
      }
    } finally {
  ...
  String _label(Category c) {
    ...
    while (parentId != null) {
      final parent = _store.categoryParentOptions.where((x) => x.id == parentId).firstOrNull;
  ...
  String? _validateSlugFormat(String slug) {
    ...
    if (!v.isAlphanumeric(noHyphens) || !v.isLowercase(noHyphens)) {
